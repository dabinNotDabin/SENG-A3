...
namespace UnitTestProject1
{
    ...
    public class BasicValidFunctionality
    {
        ...
        public void Test_PopDelivered_NoChangeDue()
        {
            ...
            expectedUnload = new VendingMachineStoredContents(); 
            var expectedCoinList2 = new List<Coin>() { five, ten, two5, two5, two5, two5, hund, hund };
            var expectedPopList2 = new List<PopCan>() { water, stuff };
            expectedUnload.CoinsInCoinRacks.Add(expectedCoinList2);
            expectedUnload.PopCansInPopCanRacks.Add(expectedPopList2);
            ...
            /* /////////Useful for printing items in unload to debug our tests///////////
             * /////////Each Test has a corresponding output (link in the Test explorer -- left -- near the bottom)

                        foreach (var list in unload.CoinsInCoinRacks)
                        {
                        ...
                        foreach (var list in unload.PopCansInPopCanRacks)
                        {
                            foreach (var item in list)
                            {
                        ...
                        foreach (var item in unload.PaymentCoinsInStorageBin)
                        {
                            Console.WriteLine(item.Value);
                        }
